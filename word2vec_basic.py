...
#
  ...
  window_size = skip_window*2+1  # [ skip_window target skip_window ]
  window_words=[]
  
  for i in range(window_size):
      window_words.append(data[data_index])
      data_index=(data_index+1)% len(data)
  for i in range(batch_size//num_skips):
       label_word=skip_window
       label_inlist=[skip_window]
       for j in range(num_skips):
           while label_word in label_inlist:
               #print(context_word)
               label_word=random.randint(0,window_size-1)
               #print(context_word)
           label_inlist.append(label_word)
           batch[i*num_skips+j]=window_words[skip_window]
           labels[i*num_skips+j,0]=window_words[label_word]
           window_words.append(data[data_index])
           data_index=(data_index+1)%len(data)
           del window_words[0]
           #print(label_words)
...
if __name__ == '__main__':
  ...
  with tf.Session(graph=graph) as sess:
    ...
    model_filepath = os.path.join(model_path, 'word2vec_srishti_%s.model'%(loss_model))
